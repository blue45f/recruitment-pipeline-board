...
현재는 애플리케이션 골격과 자동 검증 환경, 후보자 데이터 계약, Mock API와 검증된 API 클라이언트, 공용 디자인 시스템 위에 다섯 단계 보드와 200명 후보자 목록을 연결했습니다. 목록 조회 상태와 사용자 재시도를 보드 안에서 처리하고, 이름과 직무로 현재 목록을 바로 좁힐 수 있습니다.
