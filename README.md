...
현재는 애플리케이션 골격과 자동 검증 환경, 후보자 데이터 계약, Mock API와 검증된 API 클라이언트, 공용 디자인 시스템을 갖췄습니다. 그 위에 다섯 단계 보드를 연결했습니다. 표시할 데이터는 0명, 200명, 1,000명 중에서 고를 수 있고 선택값은 새로고침 뒤에도 유지됩니다. 이름과 직무로 조회한 목록을 바로 좁힐 수 있으며 후보자 카드를 열면 별도 조회 경계에서 상세 정보를 확인할 수 있습니다. 각 단계는 화면에 보이는 카드만 렌더링해 1,000명 목록에서도 DOM 크기를 일정하게 유지합니다.
